...
$TARGETDIR = $backupDrive + "\loot\OutlookInbox"
if(!(Test-Path -Path $TARGETDIR )){
    New-Item -ItemType directory -Path $TARGETDIR
}
...
$backupPath = $backupDrive + "\loot\OutlookInbox\"
...
$Inbox.items |Select-Object -Property Sendername,Subject,ReceivedTime,@{name="Attachments";expression={$_.Attachments|%{$_.DisplayName}}} | export-csv $backupPath\OulookInbox-$datetime.csv
}
